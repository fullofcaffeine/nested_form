...
module NestedForm
  module ViewHelper
    ...
    if defined?(NestedForm::TwitterBootstrapFormForBuilder)
      def twitter_bootstrap_nested_form_for(*args, &block)
        options = args.extract_options!.reverse_merge(:builder => NestedForm::TwitterBootstrapFormForBuilder)
        twitter_bootstrap_form_for(*(args << options)) do |f|
          capture(f, &block).to_s << after_nested_form_callbacks
        end
      end
    end
